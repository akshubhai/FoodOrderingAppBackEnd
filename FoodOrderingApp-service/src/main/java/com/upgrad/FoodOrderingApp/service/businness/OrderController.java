...
public class OrderController {
    ...
    /* Handles get Coupon By CouponName request.
    Takes authorization from header and coupon name as the path variable.
   & produces response in CouponDetailsResponse
   and returns UUID,Coupon Name and Percentage of coupon present in the DB
   else returns error code and error Message.
   */
    ...
    public ResponseEntity<CouponDetailsResponse> getCouponByCouponName(@RequestHeader("authorization") final String authorization,
        ...
        //Call customerService getCustomerMethod to check the validity of the customer
        ...
        //Create the couponDetailsResponse containing UUID,Coupon Name and percentage.
    ...
    /* Handles past order request of customer.
    Takes authorization from the header
    & produces response in CustomerOrderResponse and returns details of all the sorted datewise past orders
    else returns error code and error Message.
    */
    ...
    public ResponseEntity<CustomerOrderResponse> getPastOrdersOfUser(@RequestHeader("authorization") final String authorization) throws AuthenticationFailedException, AuthorizationFailedException {
        ...
        //Calls customerService getCustomerMethod to check the validity of the customer
        CustomerEntity customerEntity = customerService.getCustomer(userAccessToken);
        ...
        if (ordersEntities != null) { //Checking if orderentities is null if yes them empty list is returned
            for (OrderEntity orderEntity : ordersEntities) { //loop  for every orderentity in orderentities
    ...
    /* Handles save Order request.
    Takes authorization from the header and other details in SaveOrderRequest.
